...
def html_to_image(params: dict) -> bytes:
    ...
    with tempfile.NamedTemporaryFile(delete=False,dir="./src/app", suffix='.html') as f:
        ...
        f.file.flush()
        try:
            # Generate image from HTML file By running  google-chrome  --headless   --window-size=450,450   --disable-extensions --disable-default-apps --disable-dev-shm-usage --hide-scrollbars  --no-pdf-header-footer --screenshot=1.png htmlfile
            # subprocess.popen
            subprocess.run([
                'google-chrome',
                '--headless',
                f'--window-size={params["width"]},{params["height"]+88}',
                '--disable-extensions',
